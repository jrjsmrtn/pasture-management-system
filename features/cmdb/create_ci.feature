...
Feature: Create Configuration Items
  ...
  Scenario: Create network device via CLI
    When I run "roundup-admin -i tracker create ci name=core-switch-01 type=2 status=5 location=NetworkCloset ip_address=192.168.1.1 ports=48"
    Then the command should succeed

  @cli
  Scenario: Create server via CLI
    When I run "roundup-admin -i tracker create ci name=web-server-01 type=1 status=5 cpu_cores=4 ram_gb=16 os=RockyLinux9"
    Then the command should succeed

  @api
  Scenario: Create virtual machine via API
    Given I have a valid API token
    When I POST to "/rest/data/ci" with JSON:
      """
      {
        "name": "app-vm-01",
        "type": "6",
